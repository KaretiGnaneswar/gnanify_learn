  ...
  return (
    <div className="border-b border-neutral-200 dark:border-neutral-800 bg-gradient-to-r from-white/80 via-white/60 to-white/80 dark:from-neutral-900/80 dark:via-neutral-900/60 dark:to-neutral-900/80 backdrop-blur-md shadow-sm">
      <div className="relative mx-auto max-w-7xl px-0">
        {/* Scroll Shadows */}
        <div className="pointer-events-none absolute inset-y-0 left-0 w-6 bg-gradient-to-r from-white dark:from-neutral-900 to-transparent z-10" />
        <div className="pointer-events-none absolute inset-y-0 right-0 w-6 bg-gradient-to-l from-white dark:from-neutral-900 to-transparent z-10" />
        <div className="px-4 h-12 flex items-center gap-2 overflow-x-auto scrollbar-hide snap-x snap-mandatory">
          {items.map((it) => {
            ...
            return (
              <Link
                key={it.to}
                to={it.to}
                className={`relative text-sm font-medium px-4 py-2 rounded-md transition-all duration-200 whitespace-nowrap snap-start ${
                  isActive
